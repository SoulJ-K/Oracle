...
VALUES (900, '장채헌', '901123-1080503', 'jang_ch@kh.or.kr', '01055569512','D1', 'J7', 'S3', 4300000, 0.2,
...
--2000/01/01 이전 입사자 정보
CREATE TABLE EMP_OLD
AS SELECT EMP_ID, EMP_NAME, HIRE_DATE, SALARY
FROM EMPLOYEE
WHERE 1 =0;

CREATE TABLE EMP_NEW  -- 그 후 입사자 정보
AS SELECT EMP_ID, EMP_NAME, HIRE_DATE, SALARY
FROM EMPLOYEE
WHERE 1 =0;
...
WHEN HIRE_DATE < '2000/01/01' THEN
    INTO EMP_OLD VALUES (EMP_ID, EMP_NAME, HIRE_DATE, SALARY)
ELSE  --비교대상이 두개밖에 없을 때 IF문처럼 ELSE를 이용해서 나머지 표시 가능
    INTO EMP_NEW VALUES (EMP_ID, EMP_NAME, HIRE_DATE, SALARY)
SELECT EMP_ID, EMP_NAME, HIRE_DATE, SALARY
FROM  EMPLOYEE;

--UPDATE : 테이블에 기록된 컬럼의 값을 수정하는 구문(행 개수 변화X)
...
--UPDATE시에도 서브쿼리 가능
--평상 시 유재식 사원을 부러워하던 방명수 사원의 급여와 보너스를 유재식 사원과 동일하게 변경
...
--EMP_SALARY에서 아시아지역에 근무하는 직원의 보너스를 0.3으로 변경
--아시아에 근무하는 직원조회(사번, 이름, 급여, 보너스, 지역명)
...
WHERE DEPT_ID = 'D1';
--integrity constraint (KH.SYS_C007082) violated - child record found

--삭제 시 FOREIGN KEY 제약조건으로 컬럼삭제가 불가능할 경우 제약조건 비활성화 가능
