...
SELECT EMP_NAME, HIRE_DATE 입사일, ADD_MONTHS(HIRE_DATE, 6)
FROM EMPLOYEE;

-- EMPLOYEE 테이블에서 사원명, 입사일-오늘, 오늘-입사일 조회
-- 단, 별칭은 근무일수1, 근무일수2로 하고
-- 모두 정수처리(내림), 양수가 되도록 처리
SELECT EMP_NAME, -FLOOR(MONTHS_BETWEEN(HIRE_DATE, SYSDATE))근무일수1, FLOOR(MONTHS_BETWEEN(SYSDATE, HIRE_DATE))근무일수2
FROM employee;
...
FROM employee;
-- 2) MONTHS_BETWEEN
SELECT EMP_NAME, HIRE_DATE, TRUNC(MONTHS_BETWEEN(SYSDATE, HIRE_DATE)/12) 근무년수
FROM employee;
...
SELECT EMP_NAME, TO_CHAR(HIRE_DATE, 'YYYY"년" MON DD"일" (DY)')입사일
FROM EMPLOYEE;
...
SELECT EMP_NAME, JOB_CODE, TO_CHAR((SALARY *(1+NVL(BONUS,0))*12), 'FML999,999,999') "연봉(원)"
FROM EMPLOYEE;
...
--	-------------------------------------------------------------
SELECT COUNT (*) 전체직원수,
       COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2001', '입사'))"2001년 입사",
       COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2002', '입사'))"2002년 입사",
       COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2003', '입사'))"2003년 입사",
       COUNT(DECODE(TO_CHAR(EXTRACT(YEAR FROM HIRE_DATE)), '2004', '입사'))"2004년 입사"
FROM employee;
